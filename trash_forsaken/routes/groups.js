...
router.get('/', routeAuth, async(req,res,next) => {
    try {
        const usersGroups = await db.raw('SELECT groups.id, groups.name, users.username AS admin FROM group_users JOIN groups ON groups.id = group_users.group_id JOIN users ON users.id = groups.admin WHERE user_id = ? ', [req.userid]);/*db('groups')
...
router.get('/delete/:groupid', routeAuth, async (req,res,next) => {
    ...
    try {
        validateGroupBelonging = async (id) => {
            ...
            return (groupuser.rows.length > 0 && admin.admin === req.userid);
        }
